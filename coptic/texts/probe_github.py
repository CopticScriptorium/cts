...
def github_directory_names(corpus):
    ...
    url = (
        "https://api.github.com/repos/CopticScriptorium/corpora/contents/"
        + corpus.github.split("/")[-1]
    )
    ...
    dir_names = [
        ...
        if item["type"] == "dir" and item["name"].startswith(corpus.annis_corpus_name)
    ]
    ...
    ordered_dir_names = tuple(
        dir_name_or_blank(category) for category in ("TEI", "ANNIS", "PAULA")
    )
