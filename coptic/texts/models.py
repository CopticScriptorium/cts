...
class HtmlVisualizationFormatManager(models.Manager):
    ...
    def all(self):
        return [self._create_object_from_params(data) 
    ...
    def get(self, **kwargs):
        formats = {data['slug']: self._create_object_from_params(data) 
                  for data in HtmlVisualizationFormat.Data.FORMATS.values()}
        
        if 'slug' in kwargs:
            return formats.get(kwargs['slug'])
        if 'button_title' in kwargs:
            return next((f for f in formats.values() 
                       if f.button_title == kwargs['button_title']), None)
...
class Corpus(models.Model):
    ...
    def html_visualization_formats(self):
        return [HtmlVisualizationFormat.objects.get(slug=slug) 
    ...
    def order_by(self, *fields):
        ...
        for field in reversed(fields):
            ...
            objects = sorted(objects,
                           key=lambda x: getattr(x, field).lower(),
                           reverse=desc)
        return objects
...
class SpecialMeta(models.Model):
    ...
    class Data:
        METAS = {
            ...
            'arabic_translation': dict(name="arabic_translation", order=8, splittable=True)
        }
