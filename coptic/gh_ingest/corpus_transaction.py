...
class CorpusTransaction:
    ...
    def sort_texts(self, text_next, text_prev, text_urn):
        ...
        while scan_node is not None and scan_node not in visited:
            visited.add(scan_node)
            n_links += 1
            scan_node = scan_node.next

        # n_links should be equal to len(nodes)
        if n_links != len(nodes):
            logging.warning(
                "Insufficient data to properly order corpus based on next/prev attrs."
            )
            return

        # Now, build the new ordered list
        new_text_pairs = []
        node = start_node
        visited = set()
