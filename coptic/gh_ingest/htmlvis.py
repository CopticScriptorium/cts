...
class Directive:
	...
	def __init__(self, triggering_condition, generated_element, content=""):
		triggering_condition_dict = Directive.parse_triggering_condition(triggering_condition)
	...
	def parse_triggering_condition(cls, text):
		...
		if "=" in text:
			try:
				...
				raise HtmlGenerationException(f"Malformed target element in config: {text}") from e
...
class ValueDirective(Directive):
	...
	def apply_left(self, elt, text):
		if self._content_type == ContentTypes.VALUE:
			...
			content = content.replace("%%value%%", elt.attrs.get(self._triggered_attr_name, ""))
		else:
			content = None
...
class AnnAndValueDirective(Directive):
	...
	def applies(self, elt):
		return self._trigger_name in elt.attrs and elt.attrs[self._trigger_name] == self._trigger_val
...
class SgmlElement:
	...
	def __str__(self):
		return (f'<SgmlElement="{self.name}" open_line="{self.open_line}" close_line="{self.close_line}" '
...
def parse_config(config_text):
	...
	for line in config_text.strip().split("\n"):
		...
		trigger_type = Directive.parse_triggering_condition(triggering_condition)['type']
...
def parse_close_tag(i, line):
	...
	if len(name) != 1:
		raise HtmlGenerationException(f'Couldn\'t recognize an SGML element name on closing line {i}:\n\n\t{line}')
	return name[0]
