    ...
    def handle(self, *args, **options):
        if options["source"] == "github":
            ...
            if not options["local_repo_path"]:
                raise CommandError(
                    "The --local-repo-path argument is required when --source is 'local'."
                )
        ...
        for transaction in transactions:
            self.stdout.write(
                f"Prepared transaction for corpus {transaction.corpus_name}. Executing..."
            )
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        "Something went wrong while attempting to execute the transaction "
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully ingested corpus '{transaction.corpus_name}' with"
        ...
        self.stdout.write(
            "Your next step should be to enter the admin interface and give each "
